...
function CodePanel({ children, tag, label, code }) {
  ...
  // Handle cases where children might be an array or contain non-element nodes
  const childrenArray = Children.toArray(children).filter(child => child != null && child !== '' && child !== false)
  
  if (childrenArray.length === 1) {
    child = childrenArray[0]
  } else if (childrenArray.length > 1) {
    // Find the first valid React element
    child = childrenArray.find(child => isValidElement(child))
  }
